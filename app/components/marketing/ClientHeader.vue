...
import { 
  Menu, PhoneCall, ArrowLeft, Home, 
  Briefcase, MessageSquare, Info, Layers,
  ...
  >
    ...
    >
      ...
      <div class="hidden lg:flex items-center gap-4">
        <SharedModeToggle />
        <Button class="rounded-full px-6 gap-2 group shadow-xl shadow-primary/20 hover:scale-105 transition-all font-bold">
          مشاوره رایگان
          <PhoneCall class="w-4 h-4 group-hover:rotate-12 transition-transform" />
        </Button>
      </div>
      ...
      <div class="hidden lg:flex items-center gap-10" dir="rtl">
        ...
        <ul class="flex items-center gap-8">
          <li v-for="item in menuItems" :key="item.title">
            <NuxtLink 
              :to="item.href" 
              class="text-sm font-bold text-muted-foreground hover:text-primary transition-all relative group"
            >
              {{ item.title }}
              <span class="absolute -bottom-1 right-0 w-0 h-0.5 bg-primary transition-all duration-300 group-hover:w-full"></span>
            </NuxtLink>
          </li>
        </ul>
      </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <Sheet>
            <SheetTrigger asChild>
              <Button variant="outline" size="icon" class="rounded-2xl border-primary/20 bg-primary/5">
                <Menu class="w-6 h-6 text-primary" />
              </Button>
            </SheetTrigger>
            
            <SheetContent side="right" class="w-full sm:w-100 border-l border-border/50 bg-background/95 backdrop-blur-2xl p-0 overflow-hidden flex flex-col">
              ...
              <div class="relative h-full flex flex-col p-8 z-10" dir="rtl">
                <SheetHeader class="text-right mb-10">
                  <div class="flex justify-between items-center">
                    <SharedLogo />
                    <SheetClose class="rounded-xl p-2 hover:bg-secondary transition-colors text-muted-foreground">
                      <X class="w-6 h-6" />
                    </SheetClose>
                  </div>
                </SheetHeader>
                ...
                <div class="flex flex-col gap-3 flex-1 overflow-y-auto no-scrollbar">
                  <p class="text-[10px] font-black uppercase tracking-[3px] text-primary/60 mb-2 pr-2">دسترسی سریع</p>
                  
                  <SheetClose as-child v-for="item in menuItems" :key="item.title">
                    <NuxtLink 
                      :to="item.href"
                    ...
                    >
                      <div class="flex items-center gap-4">
                        <div class="p-2.5 rounded-xl bg-background border border-border group-hover:bg-primary group-hover:text-white transition-all duration-500">
                          <component :is="item.icon" class="w-5 h-5" />
                        </div>
                        <span class="font-black text-lg">{{ item.title }}</span>
                      </div>
                      <ArrowLeft class="w-5 h-5 opacity-0 group-hover:opacity-100 -translate-x-4 group-hover:translate-x-0 transition-all text-primary" />
                    </NuxtLink>
                  </SheetClose>
                </div>

                <div class="mt-auto pt-6 border-t border-border/40 flex items-center justify-between">
                  <span class="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Connect with us</span>
